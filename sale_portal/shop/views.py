...
def render_excel(request=None, return_url=True):
    ...
    worksheet.write('M1', 'Đường phố', merge_format)
    worksheet.write('N1', 'Số lượng Ter', merge_format)
    worksheet.write('O1', 'SLGD tuần 1 (1-7)', merge_format)
    worksheet.write('P1', 'SLGD tuần 2 (8-14)', merge_format)
    worksheet.write('Q1', 'SLGD tuần 3 (15-21)', merge_format)
    worksheet.write('R1', 'SLGD tuần 4 (22-hết tháng)', merge_format)
    worksheet.write('S1', 'SLGD tháng này', merge_format)
    worksheet.write('T1', 'SLGD tháng trước', merge_format)
    worksheet.write('U1', 'Ngày tạo', merge_format)
    ...
    for item in shops:
        ...
        worksheet.write(row_num, 12, item['street'] if item['street'] else '')
        worksheet.write(row_num, 13, item['count_ter'] if item['count_ter'] else 0)
        worksheet.write(row_num, 14, item['k1'] if item['k1'] else 0)
        worksheet.write(row_num, 15, item['k2'] if item['k2'] else 0)
        worksheet.write(row_num, 16, item['k3'] if item['k3'] else 0)
        worksheet.write(row_num, 17, item['k4'] if item['k4'] else 0)
        worksheet.write(row_num, 18, item['number_of_tran_acm'] if item['number_of_tran_acm'] else 0)
        worksheet.write(row_num, 19, item['number_of_tran_last_m'] if item['number_of_tran_last_m'] else 0)
        worksheet.write(row_num, 20, formats.date_format(item['created_date'], "SHORT_DATETIME_FORMAT") if item[
            'created_date'] else '')
